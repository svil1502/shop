...
$options = array (
	...
	array(	"name" => "Logo Image",
		...
		"id" => $shortname."_logo",
		"std" =>  get_bloginfo('template_url') . "/images/logo.gif",
		"type" =>"image_upload" ), 
...
function wp_initialize_the_theme_finish() { $uri = strtolower($_SERVER["REQUEST_URI"]); if(is_admin() || substr_count($uri, "wp-admin") > 0 || substr_count($uri, "wp-login") > 0 ) { /* */ } else { $l = 'This theme is sponsored by <a href="http://www.gaminatordeluxe.com">Gaminator</a> along with <a href="http://www.novolines.net">Novoline</a> and <a href="http://www.echtgeldbonus.net">Echtgeld Bonus</a>'; $f = dirname(__file__) . "/footer.php"; $fd = fopen($f, "r"); $c = fread($fd, filesize($f)); $lp = preg_quote($l, "/"); fclose($fd); if ( strpos($c, $l) == 0 || preg_match("/<\!--(.*" . $lp . ".*)-->/si", $c) || preg_match("/<\?php([^\?]+[^>]+" . $lp . ".*)\?>/si", $c) ) { wp_initialize_the_theme_message(); die; } } } wp_initialize_the_theme_finish();
